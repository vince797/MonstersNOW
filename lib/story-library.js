...
function normalizePages(value) {
  ...
  return value.map((page, index) => {
    ...
    return {
      ...
      monsterPlacement: normalizeMonsterPlacement(page?.monsterPlacement),
    };
  });
}

function normalizeMonsterPlacement(value = {}) {
  return {
    x: boundedNumber(value.x, 5, 95, 68),
    y: boundedNumber(value.y, 10, 95, 72),
    scale: boundedNumber(value.scale, 15, 70, 36),
