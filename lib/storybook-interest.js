...
const DEFAULT_STORYBOOK_STORY_ID = "personalized-monster-storybook";
const DEFAULT_STORYBOOK_STORY_LABEL = "Personalized Monster Storybook";
...
function buildStorybookInterestSubmission(payload = {}) {
  ...
  return {
    ...
    email,
    personalization: normalizeStorybookPersonalization(payload.personalization || payload),
    format: {
      id: variant.id,
      label: variant.label,
      priceDisplay: variant.priceDisplay,
    },
    story: normalizeStorybookTheme(payload),
    style,
    styleLabel: getMonsterStyleLabel(style),
    selectedPreviewId: normalizeShortText(payload.selectedPreviewId, 160) || null,
    monsterSubmissionId: normalizeUuid(payload.monsterSubmissionId),
...
function normalizeUuid(value) {
  ...
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(normalized)) {
    throw createStorybookInterestError("The saved monster submission is invalid.", { code: "invalid_monster_submission", status: 400 });
  }
  return normalized;
}
...
function buildStorybookInterestEmailPayload(submission, config) {
  ...
  return {
    ...
    tags: [
      ...
      { name: "story", value: normalizeTagValue(submission.story.id) },
    ],
  };
}

function normalizeStorybookTheme(payload = {}) {
  const story = typeof payload.story === "object" && payload.story ? payload.story : {};
  const id = normalizeShortText(payload.storyId || story.id, 120) || DEFAULT_STORYBOOK_STORY_ID;
  const label =
    normalizeShortText(payload.storyLabel || story.label, 160) || DEFAULT_STORYBOOK_STORY_LABEL;
...
function normalizeStorybookPersonalization(value = {}) {
  const childName = normalizeShortText(value.childName || value.child_name, 40);
  const monsterName = normalizeShortText(value.monsterName || value.monster_name, 40);

  if (!childName || !monsterName) {
    throw createStorybookInterestError("Enter the child's first name and the monster's name.", {
      code: "missing_storybook_names",
      status: 400,
    });
  }

  return { childName, monsterName };
}

function normalizeTagValue(value) {
  const normalized = String(value || DEFAULT_STORYBOOK_STORY_ID)
    .toLowerCase()
    .replace(/[^a-z0-9_-]+/g, "_")
    .replace(/^_+|_+$/g, "")
    .slice(0, 256);

  return normalized || DEFAULT_STORYBOOK_STORY_ID;
}
...
function renderStorybookInterestText(submission) {
  ...
  return [
    ...
    `Child's first name: ${submission.personalization.childName}`,
    `Monster's name: ${submission.personalization.monsterName}`,
...
function normalizePreviewAttachment(value, warnings) {
  ...
  return {
    filename: `monstersnow-${DEFAULT_STORYBOOK_STORY_ID}.${getImageExtension(mimeType)}`,
    content: base64,
  };
}
