...
function buildSignedPrintFileUrl({ request, type, submission, variant, pageCount, coverDimensions }) {
  ...
  const params = new URLSearchParams({
    ...
    child_name: normalizeParam(submission.childName || submission.child_name, "Child"),
    monster_name: normalizeParam(submission.monsterName || submission.monster_name, "Monster"),
    story_id: normalizeParam(submission.storyId || submission.story_id, ""),
    master_version: String(submission.masterVersion || submission.master_version || 1),
    monster_submission_id: normalizeParam(submission.monsterSubmissionId || submission.monster_submission_id, ""),
    selected_preview_id: normalizeParam(submission.selectedPreviewId || submission.selected_preview_id, ""),
  ...
  return `${baseUrl}/api/lulu-sandbox-storybook-order?${params.toString()}`;
}
...
function verifySignedPrintFileQuery(query) {
  ...
  return {
    ...
    monsterName: params.get("monster_name"),
    storyId: params.get("story_id"),
    masterVersion: params.get("master_version"),
    monsterSubmissionId: params.get("monster_submission_id"),
    selectedPreviewId: params.get("selected_preview_id"),
